#!/usr/bin/env python3
"""
Collect logprob AUC for missing Table 2 conditions.
Conditions to fill:
1. Qwen2.5-3B-Instruct MATH @256
2. Qwen2.5-7B-Instruct HumanEval @256

Usage: CUDA_VISIBLE_DEVICES=1 python3 -u run_logprob_collection.py
"""
...
def evaluate_code(problem, completion):
...
def main():
    print(f"Logprob Collection | {datetime.now()}", flush=True)
    device = "cuda:0"
    from transformers import AutoTokenizer, AutoModelForCausalLM
    from sklearn.metrics import roc_auc_score

    # ================================================================
    # Task 1: Qwen2.5-3B-Instruct MATH @256
    # ================================================================
    ckpt1 = CKPT_DIR / "qwen25_3b_math_256.json"
    if ckpt1.exists():
        print(f"Loading cached: {ckpt1}", flush=True)
        with open(ckpt1) as f:
            r1 = json.load(f)
    else:
        print(f"\n=== Qwen2.5-3B-Instruct MATH @256 ===", flush=True)
        model_path = "/mnt/data/pre_model/Qwen2.5-3B-Instruct"
        ...
        for i, prob in enumerate(problems):
            ...
            if len(out.scores) > 0:
                scores_tensor = torch.stack(out.scores, dim=0)  # (gen_len, vocab)
                log_probs = []
    ...
    # ================================================================
    # Task 2: Qwen2.5-7B-Instruct HumanEval @256
    # ================================================================
    ckpt2 = CKPT_DIR / "qwen25_7b_humaneval_256.json"
    if ckpt2.exists():
        print(f"Loading cached: {ckpt2}", flush=True)
        with open(ckpt2) as f:
            r2 = json.load(f)
    else:
        print(f"\n=== Qwen2.5-7B-Instruct HumanEval @256 ===", flush=True)
        ...
        with open(BASE / "humaneval_164.json") as f:
            problems = json.load(f)
        ...
        for i, prob in enumerate(problems):
            ...
            # Evaluate code
            ok = evaluate_code(prob, gen_text)
            ceiling = gen_tokens >= BUDGET - 5

            r2.append({
                "task_id": prob['task_id'],
                "ok": ok, "tok": gen_tokens,
                "mean_logprob": mean_logprob,
                "ceiling_hit": ceiling,
            })
            if (i+1) % 40 == 0:
                acc = sum(x['ok'] for x in r2)/len(r2)*100
                print(f"  [{i+1}/164] acc={acc:.1f}%", flush=True)
